...
#include <ThirdParty/redis-cpp/resp/detail/marker.h>
#include <ThirdParty/redis-cpp/resp/detail/overloaded.h>
...
{
public:
    vector(std::vector<bulk_string> values) noexcept
        : values_(values)
    {
    }
    ...
    void put(std::ostream &stream)
    {
        ...
        {
            serialization::put(stream, values_[i]);
        }
    }

private:
    std::vector<bulk_string> values_;
};
